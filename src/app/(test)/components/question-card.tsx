...
interface QuestionCardProps {
  ...
  onMarkedForReview: (value: string, answered: boolean) => void;
  onNext: () => void;
  onSubmit: () => void;
}
...
export function QuestionCard({
  ...
  const handleMarkedForReview = (selectedAnswer: string, answered: boolean) => {
    onMarkedForReview(selectedAnswer, answered)
    onNext()
  }
  ...
  return (
    <Card className="mb-8 border-none shadow-lg overflow-hidden w-2/3">
      ...
      <CardFooter className="flex justify-between bg-gray-50 py-4 px-6">
        {selectedAnswer ? (
          <Button onClick={() => handleMarkedForReview(selectedAnswer, true)} className="gap-2 bg-indigo-600">
            Mark for Review
          </Button>
        ) : (
          <Button onClick={() => handleMarkedForReview("", false)} className="gap-2 bg-amber-600">
            Mark for Review
          </Button>
        )}

        {questionNumber < totalQuestions ? (
          <Button onClick={() => handleSaveAndNext(selectedAnswer)} className="gap-2 bg-green-600">
            Save & Next
